...
echo '<br/><span class="negrita">'.$id_inf_exa.'</span>';

echo '<p class="centrado"><select id="agr" onchange="cargaAlumnos(\'inf_exa.php\')">';
...
if($agr_post)
	...
	echo '<select id="per" onchange="examenes(\''.$agr_post.'\')">';
...
{
	$sel_obs = mysql_query("select agenda.fecha,agenda.cita from agenda,horario where agenda.tipo = 'e' and horario.sesion='$agr_post' and agenda.dia=horario.dia and agenda.franja=horario.franja and agenda.docente=horario.docente and agenda.docente = '$docente' order by agenda.fecha desc");
	$num_obs = mysql_num_rows($sel_obs);
	if($num_obs>0)
	{	
	echo '<br /><br /><table class="tablacentrada"><tr><td>'.$id_fecha.'</td><td>'.$id_examen.'</td></tr>';
	for($o=0;$o<$num_obs;$o++)
		{
	...
	echo '</table>';
	echo '<br /><p><a title="'.$id_pdf.'" href="#" onclick="pdf1(\'examenes.php\',\''.$agr_post.'\',\''.$id_todo_curso.'\')"><img src="imgs/informe.png" alt="'.$id_pdf.'" /></a></p>';
	}
	else
	{
	echo '<br /><br /><p class="negrita">'.$id_no_exam.'</p>';
	}
}
...
if($agr_post && $per_post != $id_todo_curso)
{
	...
	echo '</table>';
	echo '<br /><p><a title="'.$id_pdf.'" href="#" onclick="pdf1(\'examenes.php\',\''.$agr_post.'\',\''.$per_post.'\')"><img src="imgs/informe.png" alt="'.$id_pdf.'" /></a></p>';
	}
	else
	{
	echo '<br /><br /><p class="negrita">'.$id_no_exam.'</p>';
