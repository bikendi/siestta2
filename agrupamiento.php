...
switch($registro)
	...
	case 'asi':
	//veo si hay algo
	$sel_asi=mysql_query("select * from asistencia where agrupamiento='$sesion' and codigo='$codigo' and fecha='$fecha_ing' and hini='$hini'");
	if(mysql_num_rows($sel_asi)>0)
		{
		$act_asi=mysql_query("update asistencia set dato='$dato' where agrupamiento='$sesion' and codigo='$codigo' and fecha='$fecha_ing' and hini='$hini'");
		}
	else
		{
		$ins_asi=mysql_query("insert into asistencia values('$codigo','$sesion','$fecha_ing','$hini','$dato')");
		}
	break;
	case 'not':
	...
	echo '</ol>';
	break;
	}//fin switch del registro

echo '<br/><span class="negrita">'.$id_agr.' '.$sesion.'. '.$id_fecha.': '.$nombre_dia.', '.$fecha_esp.' '.$id_horaini.': '.$hini.'</span>';

echo '<p class="centrado">';
echo '<a href="#" onclick="abreAgrup(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\')">'.$id_inf_asi.'</a>&nbsp;';
echo '<a href="#" onclick="abreAgrupNotas(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\')">'.$id_inf_not.'</a>&nbsp;';
echo '<a href="#" onclick="abreAgrupObs(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\')">'.$id_inf_obs.'</a>&nbsp;';
echo '</p><br />';
...
for($n=0;$n<$div_red;$n++)
	...
	//miramos si tiene tareas
	$sel_tarea = mysql_query("select tarea from tareas where codigo='$cod_alum' and agrupamiento = '$sesion' and fecha_ent='$fecha_ing'");
		
	$sel_dato = mysql_query("select dato from asistencia where codigo='$cod_alum' and agrupamiento='$sesion' and fecha='$fecha_ing' and hini='$hini'");
	if(mysql_num_rows($sel_dato)>0)
		{
		$reg_dato=mysql_fetch_array($sel_dato);
		echo '<li>';
		
		if(mysql_num_rows($sel_tarea)>0)
			{
			echo '<img src="imgs/advertencia.png" title="'.$id_hay_tarea.'" alt="'.$id_hay_tarea.'" />';
			}
		echo '<select style="background-color:#FF6600;" id="list_'.$n.'" onchange="regAsi(\''.$cod_alum.'\',\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$n.'\')"><option selected="selected">'.$reg_dato['dato'].'</option><option>F</option><option>R</option><option>J</option><option>A</option></select> <a href="#" onclick="abreFicha(\''.$cod_alum.'\',\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$accion.'\')" title="'.$id_verficha.'"> '.$ape_alum.'</a>, <a href="#" title="'.$id_foto.'" onclick="new LITBox(\'carga_foto_al.php?usuario='.$cod_alum.'\', {type:\'window\', overlay:true, height:190, width:120, resizable:false});">'.$nom_alum.'</a><a href="#" onclick="abreJustif(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$cod_alum.'\')" title="'.$id_just_mas.'" style="color:#F60;"> (J) </a></li>';
		}
	else
		{
		echo '<li>';
		...
		echo '<select id="list_'.$n.'" onchange="regAsi(\''.$cod_alum.'\',\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$n.'\')"><option>'.($n+1).'</option><option>F</option><option>R</option><option>J</option><option>A</option></select> <a href="#" onclick="abreFicha(\''.$cod_alum.'\',\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$accion.'\')" title="'.$id_verficha.'"> '.$ape_alum.'</a>, <a href="#" title="'.$id_foto.'" onclick="new LITBox(\'carga_foto_al.php?usuario='.$cod_alum.'\', {type:\'window\', overlay:true, height:190, width:120, resizable:false});">'.$nom_alum.'</a><a href="#" onclick="abreJustif(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$cod_alum.'\')" title="'.$id_just_mas.'" style="color:#F60;"> (J) </a></li>';
		}
	}
echo '</ul>';
echo '</td>';
...
echo '<select id="act" onchange="abreAgrupNotas1(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\')">';
if($_POST['act_post'])
	{
	echo '<option selected="selected">'.$_POST['act_post'].'</option>';
	}
echo '<option value="0">'.$id_elgact.'</option>';
$sel_per = mysql_query("select periodo,inicio,fin from periodos where inicio<'$fecha_ing' and fin>'$fecha_ing'");
...
echo '</tr>';
echo '</table><br />';
echo '<p class="centrado"><a href="#" onclick="grabaNota(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$num_alum.'\')" title="'.$id_regnotas.'"><img src="imgs/guardar.png" alt="'.$id_regnotas.'" /></a></p>';
echo '</form>';
break;
...
echo '</tr>';
echo '</table><br />';
echo '<p class="centrado"><a href="#" onclick="grabaObs(\''.$sesion.'\',\''.$fecha_ing.'\',\''.$hini.'\',\''.$num_alum.'\')" title="'.$id_regnotas.'"><img src="imgs/guardar.png" alt="'.$id_regnotas.'" /></a></p>';
echo '</form>';
break;
